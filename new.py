	...
	def face_processing(self):	
		...
		for subject in subjects:
			...
			if self.get_mouth == True:
				...
				#leftEyeHull = cv2.convexHull(mouth)
		...
		self.ard.write(struct.pack('>BBB',self.y, self.food, self.x))
		return frame
	...
	def voice(self):
		while True:
			...
			try:
				...
				if u'món 1' in comand:
					v = 0
					self.ard.write(struct.pack('>BBB',0, 0, 50))
				elif u'món 2' in comand:
					v = 1
					self.ard.write(struct.pack('>BBB',0, 1, 50))
				elif u'ăn' in comand:
					self.ard.write(struct.pack('>BBB',1, v, 50))		
